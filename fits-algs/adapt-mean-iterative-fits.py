...
def main():
    ...
    for file in fileinput.input():
        ...
        for i in range(iters):
            # Otsu's thresholding.
            print 'Applying Adapt Mean thresholding iteration '+str(i+1)+'...',
            am_thr = cv2.adaptiveThreshold(cnv_data,255,cv2.ADAPTIVE_THRESH_MEAN_C,\
                                            cv2.THRESH_BINARY,201,0)
            print 'Done.'

            # Update the FITS image to use the thresholded data.
            img_hdulist[0].data = am_thr
            
            # Write the image to a new FITS file.
            newname = name_pref + '_AM' + str(i+1) + name_ext
            ...
            cnv_data = remove_mask(cnv_data, am_thr)
